...
fastify.post<{ Body: IBody }>("/", async (req, reply) => {
	...
	try {
		...
		console.error(err);
		return reply
			.send({ message: "internal server error during create url" })
			.code(500);
	}
});
...
fastify.get<{ Params: IParam }>("/:url", async (req, reply) => {
	...
	if (!url) reply.send({ message: "enter url in param" }).code(404);
	...
	try {
		...
		if (!long_url)
			return reply
				.send({ message: "url does not found please first register it." })
				.code(404);
		...
		console.error(err);
		return reply
			.send({ message: "url doest nof found please first register it." })
			.code(500);
	}
});
